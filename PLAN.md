...
- ➕ **AI hồ sơ hành vi** (GĐ3 — `core/ai/ai_controller.gd` + `data/ai/ai_profile.gd`): `AIController` thuần logic mô phỏng từng nước đi trên `BoardLogic.clone()` rồi chấm điểm theo `AIProfile` (.tres: trọng số tile, độ khó, xu hướng skill); quyết định < 100ms, tái lập theo seed. 2 tính cách demo: aggressive (sứ quân) + defensive_healer (tên cướp). Skill do AI tự quyết qua cùng controller.
- ➕ **VFX chiến đấu**: viên tấn công → kiếm bay tới đối thủ, sát thương áp dụng theo từng đợt CLEARED (trước nổ/quét/refill) qua `TurnManager.apply_attack_wave`; chí mạng (`CRIT_CHANCE`/`CRIT_DAMAGE`) hiện số đỏ + rung. Viên máu/năng lượng/vàng/kinh nghiệm → orb màu bay về panel chủ + popup "+N". Mỗi tile vỡ có tia sáng nổ (`ClearBurst`). **Viên đặc biệt**: board logic ghi `activations` (vị trí + loại) trong event CLEARED; view phát tia quét ngang/dọc (`SweepBeam`), vòng nổ 3×3 (`BlastRing`) đúng vị trí, kể cả combo và nổ trễ. VFX là Node placeholder vẽ `_draw`, có slot texture cho asset sau.
- ➕ **Lên cấp + cây kỹ năng** (GĐ4): `Leveling` (core, đường cong `base*growth^level`) cộng XP sau khi thắng → lên cấp + điểm kỹ năng (lưu trong `PlayerProfile`). `SkillTree`/`SkillTreeNode` (.tres) + `SkillTreeService` (thuần): mở node tốn điểm + điều kiện tiên quyết, áp `StatModifier` PERMANENT (tự lưu) và/hoặc mở `SkillDefinition` mới. Màn hình cây kỹ năng (node + đường nối + popup), nút trên bản đồ; battle dùng `GameState.player_skills()` = skill gốc + skill đã mở. 5 node demo (Might/Greater Might/Precision/Vigor/War Cry).
- ➕ **Đổi cơ chế ghép (2026-06-15)**: bỏ hệ viên đặc biệt Candy Crush; match-4 = +1 lượt, match-5 = +1 lượt + sét phá 3 viên ngẫu nhiên, L/T = sét phá 3 viên quanh match. Viên cường hoá (random, 2× giá trị, viền sáng). Combo: cascade nhân giá trị mỗi đợt. Giới hạn +2 lượt/lượt đi (`TurnManager`). VFX mới: tia sét, "COMBO xN", banner "Extra turn!".
- ⏳ Còn lại: hệ thống mạng (GĐ6), polish + xuất Android (GĐ7), địa điểm/sự kiện đặc biệt.
