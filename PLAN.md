...
- ➕ **VFX chiến đấu**: viên tấn công → kiếm bay tới đối thủ, sát thương áp dụng theo từng đợt CLEARED (trước nổ/quét/refill) qua `TurnManager.apply_attack_wave`; chí mạng (`CRIT_CHANCE`/`CRIT_DAMAGE`) hiện số đỏ + rung. Viên máu/năng lượng/vàng/kinh nghiệm → orb màu bay về panel chủ + popup "+N" (cosmetic, số áp dụng ở `apply_support`). Mỗi tile vỡ có tia sáng nổ (`ClearBurst`) màu theo loại. VFX là Node placeholder vẽ `_draw`, có slot texture cho asset sau.
