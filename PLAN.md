...
- ➕ **VFX kiếm bay + chí mạng**: viên tấn công khi ghép → kiếm bay tới đối thủ, sát thương áp dụng theo từng đợt CLEARED (trước nổ/quét/refill) qua `TurnManager.apply_attack_wave`; chí mạng (`CRIT_CHANCE`/`CRIT_DAMAGE` trong StatBlock) hiện số đỏ + rung. EffectResolver tách `apply_support` (máu/năng lượng/vàng/exp) khỏi attack.
